...
if (isset($_SESSION['uname']) && isset($_SESSION['upassword'])) {
  $name = mysqli_real_escape_string($con, $_SESSION["uname"]); //escape all the characters that could damage database
  $result0 = mysqli_query($con, "SELECT profile FROM Uprofile WHERE uname = '" . $name. "'"); // Specify the column you need
  $row0 = mysqli_fetch_assoc($result0); // Use mysqli_fetch_assoc for associative arrays

  if ($row0) {
    $profile = $row0['profile']; // Access the 'profile' element if $row0 is not null
  } else {
    $errormsg = "You have not edited a profile yet! Please introduce yourself!";
  }
} else {
  header("Location: login.php");
	...
	if(!isset($_SESSION['gid'])){	
		$r = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'cookzilla' AND   TABLE_NAME   = 'CookingGroup'";
		...
		$colNum = 3; $i = 0;
   		while($row = $result->fetch_assoc()) {
			...
			if (mysqli_num_rows($resultim)==1){
				...
				$iId = "<img src= 'images/".$MyPhoto."'width = 400/>";
			} else {
				$iId = '';
			}
...
// 	$query="SELECT * FROM Recipe join RecipeTag natural join Tag WHERE tname ='".$tname."'" ;
	$query2=" SELECT * FROM recipe natural join Users WHERE uname ='".$name."' order by rTime DESC" ;
